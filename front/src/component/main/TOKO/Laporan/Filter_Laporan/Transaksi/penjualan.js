...
import {Cetak_Penjualan_Per_Item} from '../../Hasil_Cetak/Transaksi/Per Item/Penjualan'
...
const Index = (props) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try{
                const responseDataLaporan = await axios.get('http://localhost:5001/penjualan_header/show_all_laporan');
                const responseDataLaporanPerItem = await axios.post('http://localhost:5001/penjualan_header/laporan_per_item');
    ...
    const handleSearch = async () => {
        ...
        try{
            const responseDataLaporan = await axios.post('http://localhost:5001/penjualan_header/search_date',data);
    ...
    return (
        <div className="container px-0 pt-5">
            ...
            <div className="row mb-3 border-bottom">
                <h2 className="col-3">Laporan Penjualan</h2>
            ...
            <div className="row">
                ...
                <div className="col-3">
                    <ReactToPrint
                        trigger={() => <button className="btn btn-outline-success w-100">Cetak Laporan Penjualan Barang</button>}
                        content={() => componentRef2.current}
                    />
                    <div style={{ display: "none" }}><Cetak_Penjualan_Per_Item ref={componentRef2}  dataTable = {dataLaporanPerItem} dari = {dari} sampai = {sampai}/></div>
                </div>
            </div>
